# Coding Rules and Engineering Mindset

This document captures how I think about code in this repository.
It is not only a checklist of conventions. It explains the engineering priorities behind the code, the trade-offs I prefer, and the rules that should guide future changes.

The short version: I optimize for correctness, explicit boundaries, operational reliability, and code that tells the truth about the business.

## Engineering Mindset

- Treat the wallet as a financial system first, not just a CRUD service.
- Prefer simple, explicit designs over clever abstractions.
- Make business rules visible in the domain model instead of hiding them in controllers, handlers, database code, or transport-specific layers.
- Keep architectural boundaries strong enough that the dependency graph explains the system.
- Prefer consistency and traceability over short-term convenience.
- Make failure modes intentional. Retries, duplicate requests, concurrency, expired reservations, and external-service failures are normal cases in fintech systems.
- Write code so another engineer can understand the business flow without reconstructing it from infrastructure details.
- Add abstractions only when they protect a boundary, remove real duplication, or make a business concept clearer.
- Keep changes focused. Do not mix unrelated refactors with feature work.
- Prefer tests that protect rules and behavior over tests that only mirror implementation.

## Decision Priorities

When there is a trade-off, prefer this order:
